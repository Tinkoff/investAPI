...
для получения списка облигаций используется метод [getInstruments/bonds](/doctest/instruments#bonds), а для получения 
информации по конкретной облигации метод [getInstruments/bondBy](/doctest/instruments#bondby). 
...
по ссылке: [Идентификация инструментов](https://tinkoffcreditsystems.github.io/invest-openapi/faq_identification/)
...
расписание торгов на той или иной торговой площадке ([Подробнее о режимах торгов](/doctest/markets/)).
Такая информация предоставляется методом [getTradingSchedules](/doctest/instruments#tradingschedules). 
...
который возвращается в рамках подписки на получение статуса инструмента [сервиса котировок](/doctest/head-marketdata/).
...
выплат используется метод [getAccruedInterests](/doctest/instruments#getaccruedinterests). Метод позволяет получить график выплаты купонов
...
пользователя. Для получения информации о размере этого обеспечения используется метод [getFuturesMargin](/doctest/instruments#getfuturesmargin).
