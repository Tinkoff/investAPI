...
Для получения информации о возможности торговли инструментом через API в сервисе инструментов в  
методе [FindInstrument](/investAPI/instruments/#findinstrument) существует параметр `api_trade_available_flag`.
