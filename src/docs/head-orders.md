...
Для решения этой задачи можно использовать unary-метод [postOrder](/doctest/orders#postorder) 
или bidirectional-stream этого же сервиса, речь о котором пойдёт ниже. 

Unary-метод [postOrder](/doctest/orders#postorder) позволяет выставлять все 
...
[Работа с торговыми поручениями](/doctest/faq_orders/).
...
<a name="coupon"></a>
При выполнении **операций с облигациями** следует также учитывать [накопленный купонный доход](/doctest/glossary#coupon).
...
[getOrders](/doctest/orders#getorders). Обратите внимание, что данный метод 
...
[getOrderState](/doctest/orders#getorderstate). Данный метод возвращает, в 
...
выставленные заявки. Для реализации этой функциональности используется метод [cancelOrder](/doctest/orders#cancelorder).

##Bidirectional-stream работы с торговыми поручениями

TINKOFF INVEST API предоставляет так же полный набор работы с торговыми поручениями при помощи
gRPC bidirectional-stream ([подробнее про gRPC](/doctest/grpc/)), 
сообщения в потоке практически идентичны параметрам unary-методов. Для более быстрой и стабильной работы 
торгового робота рекомендуется использовать именно этот варианта взаимодействия. 